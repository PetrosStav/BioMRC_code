...
# Load checkpoint
resume_from = './bert_model_results/scibertreadermax_checkpoint_max.pth.tar'
resumed = False
...
for epoch in range(start_epoch, max_epochs):
    out_f = open('./bert_model_results/scibertreadermax_max.txt', 'a+')
    print()
    out_f.write('\n')
    print('-'*30)
    out_f.write('-'*30 + '\n')
    ...
    for tf in [1, 2, 3, 4, 5, 6, 7]:
        with open('../dataset_part{}_small.json'.format(tf)) as fi:
            data = json.load(fi)
    ...
    torch.save(state,
               './bert_model_results/scibertreadermax_checkpoint_max.pth.tar')
    out_f = open('./bert_model_results/scibertreadermax_max.txt', 'a+')
    # Evaluation Dev
    print()
    out_f.write('\n')
    ...
    with open('../dataset_part8_small.json') as fi:
        data = json.load(fi)
    ...
    if dev_acc > best_dev_acc:
        ...
        torch.save(state,
                   './bert_model_results/scibertreadermax_best_checkpoint_max.pth.tar')
    else:
    ...
    if early_stop_counter == 5:
        if model.frozen_top:
            ...
            out_f = open('./bert_model_results/scibertreadermax_max.txt',
                         'a+')
            # Early stop
    ...
    if do_test:
        ...
        with open('../dataset_part9_small.json') as fi:
            data = json.load(fi)
