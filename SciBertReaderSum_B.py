...
# Load checkpoint
resume_from = './bert_model_results/scibertreadersum_checkpoint_b.pth.tar'
resumed = False
...
for epoch in range(start_epoch, max_epochs):
    out_f = open('./bert_model_results/scibertreadersum_b.txt', 'a+')
    print()
    out_f.write('\n')
    print('-'*30)
    out_f.write('-'*30 + '\n')
    ...
    torch.save(state,
               './bert_model_results/scibertreadersum_checkpoint_b.pth.tar')
    out_f = open('./bert_model_results/scibertreadersum_b.txt', 'a+')
    # Evaluation Dev
    print()
    out_f.write('\n')
    ...
    if dev_acc > best_dev_acc:
        ...
        torch.save(state,
                   './bert_model_results/scibertreadersum_best_checkpoint_b.pth.tar')
    else:
    ...
    if early_stop_counter == 5:
        if model.frozen_top:
            ...
            out_f = open('./bert_model_results/scibertreadersum_b.txt',
                         'a+')
            # Early stop
